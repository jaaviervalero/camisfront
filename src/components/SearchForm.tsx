...
import { ADULT_SIZES, KIDS_SIZES, PRICES, PERSONALIZATION_PRICE, type Version } from '@/lib/config';
...
export default function SearchForm({ onItemAdded }: Props) {
  ...
  const unitPrice = PRICES[version] + (hasPersonalization ? PERSONALIZATION_PRICE : 0);

  function handleFileChange(f: File | null) {
    if (!f) return;
    if (f.size > MAX_FILE_MB * 1024 * 1024) { setUploadError(`Máximo ${MAX_FILE_MB} MB`); return; }
    if (!f.type.startsWith('image/'))        { setUploadError('Solo se aceptan imágenes');  return; }
    setUploadError('');
    setFile(f);
      ...
      {step === 'foto' && (
        <div className="space-y-5">
          ...
          >
            {preview ? (
              <>
                <img src={preview} alt="Vista previa" className="max-h-48 object-contain rounded-xl" />
                <span className="text-xs text-gray-500">{file?.name}</span>
                <button
                  type="button"
                  onClick={(e) => { e.stopPropagation(); setFile(null); setPreview(''); }}
                  className="absolute top-2 right-2 bg-white border rounded-full w-7 h-7 flex items-center justify-center text-gray-400 hover:text-red-500 shadow"
                >✕</button>
              </>
            ) : (
              <>
                <div className="text-5xl text-gray-300">📷</div>
                <p className="text-sm font-medium text-gray-600">Arrastrá o hacé click para elegir</p>
          ...
          <div>
            ...
            <input type="text" value={descripcion} onChange={(e) => setDesc(e.target.value)}
              maxLength={60} placeholder="Ej: Barcelona Local 2025"
              className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 outline-none" />
          </div>

          {uploadError && <p className="text-red-600 text-sm">{uploadError}</p>}

          <button type="button" onClick={() => file && setStep('configure')} disabled={!file}
      ...
      {step === 'configure' && (
        <div className="space-y-5">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setStep('foto')} className="text-sm text-gray-500 hover:text-gray-700 underline">
              ← Cambiar foto
            </button>
          </div>
          ...
          {preview && (
            <div className="flex gap-4 items-center">
              <img src={preview} alt="Camiseta" className="w-20 h-20 object-contain rounded-xl border bg-gray-50 shrink-0" />
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {sizes.map((s) => (
                <button key={s} type="button" onClick={() => setTalla(s)}
                  className={`w-12 py-1.5 rounded-lg text-sm font-medium border transition ${
                    talla === s ? 'bg-green-600 text-white border-green-600' : 'bg-white text-gray-700 border-gray-300 hover:border-green-500'
                  }`}>
                  {s}
                </button>
              ))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Nombre <span className="text-gray-400 font-normal">(opc.)</span>
              </label>
              <input type="text" value={nombre} onChange={(e) => setNombre(e.target.value)}
                maxLength={20} placeholder="VINICIUS"
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 outline-none uppercase" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Dorsal <span className="text-gray-400 font-normal">(opc.)</span>
              </label>
              <input type="text" value={dorsal}
                onChange={(e) => setDorsal(e.target.value.replace(/\D/g, '').slice(0, 2))}
                maxLength={2} placeholder="7"
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 outline-none" />
            </div>
          </div>
          ...
          <button type="button" onClick={handleAddToCart} disabled={uploading}
